...
As configuration we choose the option "Simple input field" from the dropdown above the configuration textarea.
...
"File Abstraction Layer" from the ``TYPE: FAL`` group. It inserts a TCA ``type=file`` configuration with options such
as the minimum and maximum number of images.
...
.. hint::
   The size of images displayed in the Simple Backend View can be adjusted with PageTS.
...
.. note::
   **DCE container is no replacement for EXT:container!**
