...
- **New TCA field name:**
  Name of column in database and TCA. You are free to choose the name. Nothing gets prepended.
  It is recommended to `use_underscores` instead of `usingCamelCase`.
- **New TCA field type:**
  This is the type of the field in database. Example: `varchar(255) DEFAULT '' NOT NULL`.
  You can also use the keyword `auto`. DCE will choose a proper sql field type based on choosen configuration type in
  flexform.
...
Everytime you change a content element based on DCE with TCA mappings, the TCA values will get written,
